...
vim.api.nvim_create_user_command("DelMarks", function()
	-- 提供选择框让用户选择删除标记的方式
	vim.ui.select({ "All marks", "Line marks", "Single mark" }, {
		prompt = " Please select!",
	}, function(selected)
		if not selected then
			vim.notify("操作取消", vim.log.levels.WARN)
			return
		end
		...
		local function delete_marks(is_local)
			...
			if #deleted_marks > 0 then
				...
				vim.notify("未找到可删除的标记.", vim.log.levels.WARN)
			end
		end

		if selected == "All marks" then
			vim.cmd("delmarks a-z A-Z")
			vim.notify("所有标记已删除!", vim.log.levels.INFO)
		elseif selected == "Line marks" then
			delete_marks(true) -- 删除当前行局部标记
			delete_marks(false) -- 删除全局标记
		elseif selected == "Single mark" then
			local mark = vim.fn.input("输入要删除的标记: ")
			if mark ~= "" then
				vim.cmd("delmarks " .. mark)
				vim.notify("已删除标记: " .. mark, vim.log.levels.INFO)
			else
				vim.notify("未输入标记，操作已中止.", vim.log.levels.ERROR)
			end
		else
