...
FoldText = function()
	...
	local padding = foldStartLine:match('Padding:%s*"(%d+)"') ~= nil
			and tonumber(foldStartLine:match('Padding:%s*"(%d+)"'))
		or 1
	...
	local fillCharLen = totalVirtualColumns
		- vim.fn.strchars(string.rep(border, padding) .. icon .. string.rep(border, gap) .. title .. number .. border)

	local _out = string.rep(border, padding)
		.. icon
		.. string.rep(border, gap)
		.. title
		.. string.rep(border, fillCharLen)
		.. number
		.. border
