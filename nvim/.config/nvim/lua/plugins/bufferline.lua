	...
	config = function()
		...
		bufferline.setup({
			options = {
				...
				diagnostics_update_in_insert = false, -- 插入模式下不更新诊断提示
		...
		-- 快捷键设置
		for n = 1, 9 do
			map("n", "g" .. n, function() -- 跳转到第n个buffer
				require("bufferline").go_to(n, true)
			end, { desc = "[Bufferline] 跳转到第" .. n .. "个buffer" })
		end

		map("n", "<TAB>", "<cmd>BufferLineCycleNext<cr>", { desc = "下一个缓冲区" })
		map("n", "<S-TAB>", "<cmd>BufferLineCyclePrev<cr>", { desc = "上一个缓冲区" })
		map("n", "<leader>tp", "<cmd>BufferLineTogglePin<cr>", { desc = "图钉📌" })
		map("n", "<leader>tg", ":BufferLinePick<CR>", { desc = "跳转到任意可见标签" })
		map("n", "<leader>tx", ":BufferLinePickClose<CR>", { desc = "删除任意可见标签" })
		map("n", "<leader>td", "<cmd>BufferLineCloseOthers<cr>", { desc = "删除其他所有buffers" })
	end,
}
