...
-- 4. Create function to animate the cursor
...
-- 5. Bind extmark function to CursorMoved autocommand (with command mode detection)
vim.api.nvim_create_autocmd("CursorMoved", {
	callback = function()
		-- 检查是否在命令行模式，如果是则跳过动画
		local mode = vim.api.nvim_get_mode().mode
		if mode == "c" or mode == "cv" or mode == "ce" or mode == "r" or mode == "rm" then
			return
		end
